...
public class EmlRealConstSearch extends CommonRealSearch {
    ...
    public static double evaluateRank(int leaves, long rank, boolean allowInf) {
        ...
        double leftVal = evaluateRank(leftLeaves, leftRank, allowInf);
        double rightVal = evaluateRank(leaves - leftLeaves, rightRank, allowInf);
        return emlEval(leftVal, rightVal, allowInf);
    }
    ...
    public static void main(String[] args) {
        ...
        // ターゲット: 0.5 (1/2)
        double target = 0.5;
        boolean allowInf = true; // 無限大経由の探索を許可
        double threshold = 1e-10;

        System.out.println("Searching for Constant Real target: " + target + " (allow_inf=" + allowInf + ")");
        ...
        for (int K = 1; K <= 43; K += 2) {
            ...
            for (long rank = 0; rank < totalShapes; rank++) {
                double res = evaluateRank(leaves, rank, allowInf);
                if (!Double.isNaN(res) && Math.abs(res - target) < threshold) {
